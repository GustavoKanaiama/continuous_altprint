...
# Verifica se o script está sendo executado diretamente (não importado como módulo/lib)
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    # shape = Polygon([(0,0), (10,0), (10, 10), (0, 10)])
    # Define uma lista de coordenadas que representa a borda externa da forma
    border = [(0, 0), (10, 0), (10, 10), (20, 10), (20, 5),
              (30, 5), (30, 40), (10, 25), (0, 30)]
    holes = [[(5, 15), (15, 15), (15, 20), (5, 20)],
             [(20, 25), (25, 25), (25, 30)]]  # Define uma lista de buracos (interiores) representados por listas de coordenadas
    # holes = [[(5, 15), (15, 15), (15, 20), (5, 20)]]
    # Cria um polígono com a borda externa e os buracos especificados
    poly = Polygon(border, holes)
    gap = 2
    thres = 0
    angle = 20

    # Gira o polígono em um ângulo especificado em torno da origem (0, 0)
    r_shape = rotate(poly, angle, origin=(0, 0))
    # Translada o polígono rotacionado para que seu canto inferior esquerdo esteja na origem (0, 0)
    tr_poly = translate(r_shape, -r_shape.bounds[0], -r_shape.bounds[1])

    # Gera caminhos de preenchimento reticulado para o polígono transladado
    paths = rectilinear_fill(tr_poly, gap, 0, thres)

    fig, ax = plt.subplots()  # Cria uma figura e um conjunto de eixos para plotagem
    # Obtém as coordenadas da borda externa do polígono transladado
    x, y = tr_poly.exterior.xy
    # Plota a borda externa do polígono
    ax.plot(x, y, color="black", linewidth=3)
    # Itera através dos buracos (interiores) do polígono transladado
    for hole in tr_poly.interiors:
        x, y = hole.xy  # armazena as coordenas x e y de cada buraco
        ax.plot(x, y, color="black", linewidth=3)  # Plota cada buraco

    for path in paths.geoms:  # Itera através dos caminhos de preenchimento reticulado
        x, y = path.xy
        ax.plot(x, y, linewidth=2)  # Plota cada caminho com espessura 2
    ax.axis("equal")  # Define a proporção dos eixos como igual
    plt.show()  # Exibe o gráfico que representa esse prenchimento
